...
A real-time collaborative drawing application where multiple users can draw together on a shared canvas. Built over 3 days as a learning project to explore WebSocket synchronization and Canvas APIs.
...
| Day 3 | Undo/redo system, UI polish, bug fixes, documentation |
...
### Mobile Support
- **Basic touch support** - I added touch event handlers, but the experience isn't optimized for mobile. Drawing works, but the UI might feel cramped on small screens.
...
### Canvas Size
- **Fixed canvas dimensions** - the canvas resizes to fit the container, but there's no infinite canvas or pan/zoom functionality.

### Browser Compatibility
- **Modern browsers only** - tested on Chrome and Firefox. Should work on Edge. Probably won't work on IE11 (but who uses that anymore?).
...
│   └── state-manager.js  # Stroke history & undo/redo
├── package.json
├── README.md
